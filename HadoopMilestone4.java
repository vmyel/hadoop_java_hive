...
public class HadoopMilestone4 {

    public static class StatTuple implements Writable {
        long len; long count; String word;
        public StatTuple() { len=0; count=0; word=""; }
        public StatTuple(long l, long c, String w) { len=l; count=c; word=w; }
        public void write(DataOutput out) throws IOException { out.writeLong(len); out.writeLong(count); out.writeUTF(word); }
        public void readFields(DataInput in) throws IOException { len=in.readLong(); count=in.readLong(); word=in.readUTF(); }
    }

    public static class StatMapper extends Mapper<Object, Text, Text, StatTuple> {
        ...
        private void analyze(String book, String text, Context ctx) throws IOException, InterruptedException {
            ...
            for (String t : text.split("\\s+")) {
                String w = t.replaceAll("[^a-zA-Z]", "");
                if(!w.isEmpty()) {
                    StatTuple tup = new StatTuple(w.length(), 1, w);
                    ctx.write(new Text(book), tup);
                    ctx.write(new Text("Whole Bible"), tup);
                }
            }
        }
    ...
    public static class StatReducer extends Reducer<Text, StatTuple, Text, Text> {
        ...
        public void reduce(Text key, Iterable<StatTuple> values, Context ctx) {
            long tLen=0, tCount=0; String longest="";
            for(StatTuple v : values) { tLen+=v.len; tCount+=v.count; if(v.word.length()>longest.length()) longest=v.word; }
            double avg = (tCount==0)?0:(double)tLen/tCount;
            results.put(key.toString(), String.format("Avg: %.2f | Longest: %s", avg, longest));
        }

        @Override protected void cleanup(Context ctx) throws IOException, InterruptedException {
            printHeader(ctx, "MILESTONE 4a");
            for(String b : BOOKS) if(results.containsKey(b)) write(ctx, String.format("%-25s : %s", b, results.get(b)));
            ctx.write(new Text("\n\n"), new Text(""));
            printHeader(ctx, "MILESTONE 4b");
    ...
    public static void main(String[] args) throws Exception {
        ...
        Job j = Job.getInstance(c, "M4");
        j.setJarByClass(HadoopMilestone4.class); j.setMapperClass(StatMapper.class); j.setReducerClass(StatReducer.class);
        j.setMapOutputValueClass(StatTuple.class); j.setOutputKeyClass(Text.class); j.setOutputValueClass(Text.class); j.setNumReduceTasks(1);
